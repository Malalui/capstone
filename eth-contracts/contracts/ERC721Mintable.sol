...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(address(newOwner) != address(0),"Invalid address");
        _owner = newOwner;
...
contract Pausable is Ownable{
  ...
  event Paused(address whoTriggered);
  event Unpaused(address whoTriggered);
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender == getOwner() || isApprovedForAll(_tokenOwner[tokenId], msg.sender) == true);
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        emit Transfer(getOwner(), to, tokenId);  // FIXME: who is the original owner, the contract?
    }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, "Owner of the token different from from address");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "Token has to be transfered to valid address");
...
contract PatriciaERC721Token is ERC721Metadata{
  bytes4 private constant _INTERFACE_ID_PATRICIA = 0x0177c4aa;
  constructor() ERC721Metadata ("PatriciaERC721Token", "PTC", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") public
      {
          _registerInterface(_INTERFACE_ID_PATRICIA);
      }
